  ...
  for(i = 0.0; i < xLabels->stringArrayLength; i = i + 1.0){
    if(xLabelPriorities->numberArray[(int)(i)] == p){
      ...
      if( !currentOverlaps  && p == 1.0){
        pbDrawText(canvas, r->x1 + padding, r->y1 + padding, text, textLength, gridLabelColor);
  ...
  if( !overlap  && p != 1.0){
    for(i = 0.0; i < xGridPositionsLength; i = i + 1.0){
      ...
      if(xLabelPriorities->numberArray[(int)(i)] == p){
        ...
        pbDrawText(canvas, r->x1, r->y1, text, textLength, gridLabelColor);
...
void pbDrawText(RGBABitmapImage *canvas, double x, double y, wchar_t *text, size_t textLength, RGBA *color){
...
void DrawTextUpwards(RGBABitmapImage *canvas, double x, double y, wchar_t *text, size_t textLength, RGBA *color){
  ...
  pbDrawText(buffer, 0.0, 0.0, text, textLength, color);
...
_Bool DrawScatterPlotFromSettings(RGBABitmapImageReference *canvasReference, ScatterPlotSettings *settings, StringReference *errorMessage){
  ...
  if(success){
    ...
    /* Draw title */
    pbDrawText(canvas, floor(settings->width/2.0 - GetTextWidth(settings->title, settings->titleLength)/2.0), floor(yPadding/3.0), settings->title, settings->titleLength, GetBlack());
...
if(settings->yAxisLeft){
    ...
    pbDrawText(canvas, floor(originTextXPixels - GetTextWidth(settings->xLabel, settings->xLabelLength)/2.0), yPixelMax + axisLabelPadding, settings->xLabel, settings->xLabelLength, GetBlack());
...
_Bool DrawBarPlotFromSettings(RGBABitmapImageReference *canvasReference, BarPlotSettings *settings, StringReference *errorMessage){
  ...
  if(success){
    ...
    /* Draw title */
    pbDrawText(canvas, floor(ImageWidth(canvas)/2.0 - GetTextWidth(settings->title, settings->titleLength)/2.0), floor(yPadding/3.0), settings->title, settings->titleLength, GetBlack());
    ...
    for(n = 0.0; n < bs; n = n + 1.0){
      ...
      pbDrawText(canvas, floor(x), ImageHeight(canvas) - yPadding + 20.0, label, labelLength, gridLabelColor);
...
RGBABitmapImage *pbCopyImage(RGBABitmapImage *image){
